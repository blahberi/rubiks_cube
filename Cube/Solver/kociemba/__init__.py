...
def solve(cube, patternstring=None, max_depth=24):
    """
    ...
    """
    temp = cube.get_cube_colors()
    colors_to_side = {'w':'U', 'o':'L', 'g':'F', 'r':'R', 'b':'B', 'y':'D'}
    cubestring = ''
    for color in temp:
        cubestring += colors_to_side[color]

    cubestring_list = []
    for i in range(1, 7):
        slice1 = i*9 - 9
        slice2 = i*9
        cubestring_list.append(cubestring[slice1:slice2])

    cubestring = ''
    order = [0, 3, 2, 5, 1, 4]
    for i in order:
        cubestring += cubestring_list[i]

    solution = list(_solve(cubestring, patternstring, max_depth))
